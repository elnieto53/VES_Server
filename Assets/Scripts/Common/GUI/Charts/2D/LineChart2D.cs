namespace VESCharts
{
    ...
    public class LineChart2D : IChart2D
    {
        ...
        public LineChart2D(ChartAxes2D axes, float lineWidth, int depth)
        {
            this.axes = axes;
            ...
            meshRenderer.material = (Material)Resources.Load(ResourcesPath.Material.black);
        ...
        public void Draw()
        {
            ...
            {
                float angle = Mathf.Atan((points[i + 1].y - points[i].y) / (points[i + 1].x - points[i].x));
                Vector2 offset = new Vector2(lineWidth * Mathf.Sin(angle), -lineWidth * Mathf.Cos(angle));
                vertices[i * 4] = axes.GetCoordinates(points[i]) + offset;
                vertices[i * 4 + 1] = axes.GetCoordinates(points[i + 1]) + offset;
                vertices[i * 4 + 2] = axes.GetCoordinates(points[i + 1]) - offset;
                vertices[i * 4 + 3] = axes.GetCoordinates(points[i]) - offset;
        ...
        public Chart2DRange GetRange()
        {
            if (points.Count == 0)
                return null;
            return new Chart2DRange(points.Min(p => p.x), points.Max(p => p.x), points.Min(p => p.y), points.Max(p => p.y));
        }
